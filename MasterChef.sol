...
// MasterChef is the master of BalL Token (BALL). He can make BALL and he is a fair guy.
//
...
// contract once $BALL is sufficiently distributed and the community can show to govern itself.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BALLs
    ...
    // Initial emission rate: 1 BALL per block.
    uint256 public constant INITIAL_EMISSION_RATE = 1 ether;
    // Minimum emission rate: 0.5 BALL per block.
    ...
    constructor(
        ...
        ballPerBlock = INITIAL_EMISSION_RATE;
    ...
    function pendingBall(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 ballReward =
                multiplier.mul(ballPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accBallPerShare = accBallPerShare.add(
                ballReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accBallPerShare = pool.accBallPerShare.add(
            ballReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accBallPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
    ...
    // Deposit LP tokens to MasterChef for BALL allocation with referral.
    ...
    // Reduce emission rate by 3% every 14,400 blocks ~ 12hours till the emission rate is 0.5 BALL.
    ...
    function updateEmissionRate() public {
        ...
        require(ballPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = ballPerBlock;
        ...
        if (newEmissionRate >= ballPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = ballPerBlock;
        ballPerBlock = newEmissionRate;
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            ball.mint(referrer, refBonusEarned);
